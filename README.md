...
| Package | Purpose |
|---|---|
| [`clubcpg`](https://pypi.org/project/clubcpg/) | Provides `BamFileReadParser`, used for read parsing, CpG correction, and matrix construction |
| `pysam` | BAM/CRAM I/O |
...
Process every CRAM/BAM file in a directory against a CpG list:
...
2. **Per-bin extraction** (`calculate_bin_coverage`, run in a worker process):
   - Opens the CRAM/BAM via `BamFileReadParser` (CRAM reference handling is managed internally by the parser — see [Troubleshooting](#troubleshooting)).
...
**CRAM files fail to open / reference-related errors:**
This script does not pass a reference genome explicitly; CRAM decoding is handled internally by `BamFileReadParser` via `pysam`/`htslib`. If your CRAMs don't carry a resolvable embedded/cached reference, set the standard htslib environment variables before running, e.g.:

```bash
export REF_PATH=/path/to/reference/cache/%2s/%2s/%s
export REF_CACHE=/path/to/reference/cache/%2s/%2s/%s
```

or ensure the original reference FASTA (with a matching `.fai` index) is available and discoverable on the system.
